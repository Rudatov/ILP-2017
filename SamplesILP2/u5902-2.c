#include <stdio.h>
#include <stdlib.h>
#include "ilp.h"

/* Global variables */
ILP_Object pseudosequence;
ILP_Object foo;

/* Global prototypes */
ILP_Object ilp__pseudosequence (ILP_Closure ilp_useless,
				ILP_Object one1, ILP_Object two2);
ILP_Object ilp__foo (ILP_Closure ilp_useless, ILP_Object x3);

/* Global functions */

ILP_Object
ilp__pseudosequence (ILP_Closure ilp_useless,
		     ILP_Object one1, ILP_Object two2)
{
  return two2;
}

struct ILP_Closure pseudosequence_closure_object = {
  &ILP_object_Closure_class,
  {{ilp__pseudosequence,
    2,
    {NULL}}}
};

ILP_Object
ilp__foo (ILP_Closure ilp_useless, ILP_Object x3)
{
  {
    ILP_Object ilptmp215;
    ILP_Object ilptmp216;
    ilptmp215 = ILP_Integer2ILP (2);
    ilptmp216 = x3;
    return ILP_Times (ilptmp215, ilptmp216);
  }
}

struct ILP_Closure foo_closure_object = {
  &ILP_object_Closure_class,
  {{ilp__foo,
    1,
    {NULL}}}
};


ILP_Object
ilp_program ()
{
  {
    ILP_Object ilptmp217;
    ilptmp217 = ILP_Integer2ILP (12);

    {
      ILP_Object y4 = ilptmp217;
      {
	ILP_Object ilptmp218;
	{
	  ILP_Object ilptmp219;
	  ILP_Object ilptmp220;
	  {
	    ILP_Object ilptmp221;
	    {
	      ILP_Object ilptmp222;
	      ilptmp222 = y4;
	      ilptmp221 = ilp__foo (NULL, ilptmp222);
	    }
	    ilptmp219 = (y4 = ilptmp221);
	  }
	  ilptmp220 = y4;
	  ilptmp218 = ilp__pseudosequence (NULL, ilptmp219, ilptmp220);
	}
	return ilp__foo (NULL, ilptmp218);
      }

    }
  }

}

static ILP_Object
ilp_caught_program ()
{
  struct ILP_catcher *current_catcher = ILP_current_catcher;
  struct ILP_catcher new_catcher;

  if (0 == setjmp (new_catcher._jmp_buf))
    {
      ILP_establish_catcher (&new_catcher);
      return ilp_program ();
    };
  return ILP_current_exception;
}

int
main (int argc, char *argv[])
{
  ILP_START_GC;
  ILP_print (ilp_caught_program ());
  ILP_newline ();
  return EXIT_SUCCESS;
}
